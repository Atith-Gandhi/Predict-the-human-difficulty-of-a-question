from transformers import Trainer, TrainingArguments, ConvBertForSequenceClassification, ConvBertTokenizerFast
...
train_dataset_tmp = train_dataset_tmp.map(lambda example: {'label': [False if example['difficulty'] == 'School' else True]})
test_dataset_tmp = test_dataset_tmp.map(lambda example: {'label': [False if example['difficulty'] == 'School' else True]})
